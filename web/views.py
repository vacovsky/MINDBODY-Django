...
def sale_service(request):
    ...
    try:
        for i in datacharts:
            ReportsCacheModel.objects.filter(
                datapull_datestamp=datetime.datetime.today(), chart_name=i)[0]
...
def client_service(request):
    ...
    if 'userid' in request.session:
        result = ConsumerCredentialManager(
            request.session['username'], request.session['password']).result
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if request.session['userid'] == None:
                context[
                    'errormsg'] = "The username or password you provided was invalid.  Please try again."
...
def create_account(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if request.session['userid'] == None:
                context[
                    'errormsg'] = "An issue was encountered while creating your account.  This message should handle errors better and based off the actual API Call results."
            else:
